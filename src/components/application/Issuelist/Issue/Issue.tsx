...
export default function EachIssue(props: IssueProps): ReactElement {
  ...
  return (
    <div className={`issue${props.data.completed ? " striked" : ""}`}>
      <div
        className={`issue_heading ${props.data.priority.toLowerCase()}_priority_head`}
        onClick={toggleIssueDetails}
      >
        <div className="issue_deadline">Due on:&emsp;{props.data.deadline}</div>
        <span className="bar"></span>
        <div className="issue_proj_name">
          <b>{props.data.projName}</b>
        </div>
      ...
      {IssueListShow ? (
        <div
          className={`issue_content ${props.data.priority.toLowerCase()}_priority_content`}
        >
          <div>
            <div className="issue_assigned">
              Assigned to:{" "}
              <span className="assigned_name">{props.data.assigned}</span>
            </div>
            <div className="issue_content">{props.data.details}</div>
          </div>
          <div className="functions">
            ...
            >
              <FormControl
                ...
                style={{
                  padding: 15,
                  background: "rgba(123, 235, 255, 0.651)",
                }}
              >
                <FormLabel
                  component="legend"
                ...
                >
                  <FormControlLabel
                    value="High"
                    control={
                      <Radio style={{ color: "rgba(255, 151, 109, 0.699)" }} />
                    }
                    label="High"
                  />
                  <FormControlLabel
                    value="Medium"
                    control={
                      <Radio style={{ color: "rgba(245, 213, 32, 0.678)" }} />
                    }
                    label="Medium"
                  />
                  <FormControlLabel
                    value="Low"
                    control={
                      <Radio style={{ color: "rgba(255, 136, 255, 0.678)" }} />
                    }
                    label="Low"
                  />
                </RadioGroup>
